...
! Local variables
      integer :: i, j
      real :: a1,a2,b1,b2
      real, dimension(ni,nj-1) :: L1
      real, dimension(ni-1,nj) :: L2  
...
dmin = 1000.0
  do i=1,ni
    do j=1,nj-1
      ...
      L1(i,j) = sqrt(dlix(i,j)**2.0+dliy(i,j)**2.0)
      if (L1(i,j)<dmin) dmin = L1(i,j)
...
! INSERT your code here
  ...
  do i = 1,ni-1
    do j = 1,nj
      ...
      ! Calculate magnitude of cell edge lengths and set dmin if smaller
      L2(i,j) = sqrt(dljx(i,j)**2.0+dljy(i,j)**2.0)
      if (L2(i,j)<dmin) dmin = L2(i,j)
...
! Insert your code here (or in the do loops above).
    ...
    do i = 1,ni-1
      do j = 1,nj-1
        dmin1(i,j) = min(L1(i,j),L1(i+1,j),L2(i,j),L2(i,j+1))
