      subroutine smooth_corr(prop, corr_prop)
...
! Local stuff
      ...
      real  ::  sf, sfm1, avg, avg1, avgnj, corrnew, corrnew1, corrnewnj
      integer  ::  i, j, ip1, im1
...
! On the surfaces j=1 and j=nj take the average as shown below.
         ...
         avg1  = (prop(im1,1)+prop(ip1,1)+2.0*prop(i,2)-prop(i,3))/3.0
         corrnew1 = fcorr*(prop(i,1) - avg1)
         corr_prop(i,1) = 0.99*corr_prop(i,1) + 0.01*corrnew1
         
         avgnj = (prop(im1,nj) + prop(ip1,nj) + 2.0*prop(i,nj-1)    &
              -  prop(i,nj-2))/3.0
         corrnewnj = fcorr*(prop(i,nj) - avgnj)
         corr_prop(i,nj) = 0.99*corr_prop(i,nj) + 0.01*corrnewnj
